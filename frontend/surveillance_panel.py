...
class SurveillancePanel(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        self.camera_tree = ttk.Treeview(
            camera_list_frame, columns=("name", "status", "password"), show="headings", height=10
        )
        ...
        scrollbar_cameras = ttk.Scrollbar(
            camera_list_frame, orient=tk.VERTICAL, command=self.camera_tree.yview
        )
        ...
        ttk.Button(
            control_frame, text="Enable Camera", command=self.enable_camera, width=18
        ).pack(pady=2)
        ttk.Button(
            control_frame, text="Disable Camera", command=self.disable_camera, width=18
        ).pack(pady=2)
        ttk.Button(
            control_frame, text="Set Password", command=self.set_camera_password, width=18
        ).pack(pady=2)
        ttk.Button(
            control_frame, text="Delete Password", command=self.delete_camera_password, width=18
        ).pack(pady=2)

        floor_plan_btn = ttk.Button(
            control_frame, text="View Floor Plan", command=self.show_floor_plan, width=18
        )
        floor_plan_btn.pack(pady=5)

        thumbnail_btn = ttk.Button(
            control_frame, text="View Thumbnails", command=self.show_thumbnails, width=18
        )
        ...
        pan_zoom_frame = ttk.LabelFrame(right_frame, text="Pan/Zoom Controls", padding=10)
        ...
        ttk.Button(
            zoom_frame, text="Zoom In", command=lambda: self.adjust_zoom(1.2), width=10
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            zoom_frame, text="Zoom Out", command=lambda: self.adjust_zoom(0.8), width=10
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
        ...
        ttk.Button(pan_frame, text="↑", command=lambda: self.adjust_pan(0, -10), width=5).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(pan_frame, text="↓", command=lambda: self.adjust_pan(0, 10), width=5).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(pan_frame, text="←", command=lambda: self.adjust_pan(-10, 0), width=5).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(pan_frame, text="→", command=lambda: self.adjust_pan(10, 0), width=5).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(pan_frame, text="Reset Pan", command=self.reset_pan, width=12).pack(
            side=tk.LEFT, padx=2
        )
    ...
    def on_camera_select(self, event):
        ...
        if cam_id in self.cameras:
            ...
            if not camera["enabled"]:
                messagebox.showinfo("Camera Disabled", "This camera is currently disabled")
                return
    ...
    def display_camera_view(self, cam_id):
        ...
        if image_path and image_path.exists():
            try:
                ...
                self.camera_canvas.create_image(320, 240, image=self.camera_image, anchor=tk.CENTER)
                self.camera_canvas.create_text(
                    10,
                    10,
                    text=camera["name"],
    ...
    def enable_camera(self):
        ...
        if cam_id in self.cameras:
            ...
            self.refresh_camera_list()
            messagebox.showinfo("Success", f"Camera '{self.cameras[cam_id]['name']}' enabled")
    ...
    def disable_camera(self):
        ...
        if cam_id in self.cameras:
            ...
            self.refresh_camera_list()
            messagebox.showinfo("Success", f"Camera '{self.cameras[cam_id]['name']}' disabled")

    def set_camera_password(self):
        selected = self.camera_tree.selection()
        if not selected:
            messagebox.showinfo("No Selection", "Please select a camera to set password")
            return
    ...
    def delete_camera_password(self):
        selected = self.camera_tree.selection()
        if not selected:
            messagebox.showinfo("No Selection", "Please select a camera to delete password")
            return
        ...
        if cam_id in self.cameras:
            ...
            result = messagebox.askyesno(
                "Delete Password",
                f"Are you sure you want to delete the password for {self.cameras[cam_id]['name']}?",
            )
            if result:
    ...
    def show_floor_plan(self):
        ...
        for cam_id, (x, y) in camera_positions.items():
            if cam_id in self.cameras:
                ...
                canvas.create_text(
                    x, y - 20, text=camera["name"], font=("Arial", 10), anchor=tk.CENTER
                )
    ...
    def show_thumbnails(self):
        ...
        for cam_id, camera in sorted(self.cameras.items()):
            if camera["enabled"]:
                thumb_frame = ttk.LabelFrame(scrollable_frame, text=camera["name"], padding=5)
                ...
                ttk.Button(
                    thumb_frame,
                    text="View",
                    command=lambda cid=cam_id: self.view_camera_from_thumbnail(cid, thumbnail_window),
                ).pack(pady=5)
