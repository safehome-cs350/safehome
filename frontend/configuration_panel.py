...
class ConfigurationPanel(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        row = 0
        ttk.Label(settings_frame, text="System Name:").grid(row=row, column=0, sticky=tk.W, pady=5, padx=5)
        ...
        row += 1
        ttk.Label(settings_frame, text="Master Password:").grid(row=row, column=0, sticky=tk.W, pady=5, padx=5)
        ...
        self.master_password_entry = ttk.Entry(
            password_frame, textvariable=self.master_password_var, width=25, show="*"
        )
        self.master_password_entry.pack(side=tk.LEFT)

        ttk.Button(password_frame, text="Change", command=self.change_master_password, width=10).pack(
            side=tk.LEFT, padx=5
        )

        row += 1
        ttk.Label(settings_frame, text="Guest Password:").grid(row=row, column=0, sticky=tk.W, pady=5, padx=5)
        ...
        self.guest_password_entry = ttk.Entry(
            guest_password_frame, textvariable=self.guest_password_var, width=25, show="*"
        )
        self.guest_password_entry.pack(side=tk.LEFT)

        ttk.Button(guest_password_frame, text="Change", command=self.change_guest_password, width=10).pack(
            side=tk.LEFT, padx=5
        )

        row += 1
        ttk.Label(settings_frame, text="Delay Time (seconds):").grid(row=row, column=0, sticky=tk.W, pady=5, padx=5)
        ...
        ttk.Entry(delay_frame, textvariable=self.delay_time_var, width=10).pack(side=tk.LEFT)
        ttk.Label(delay_frame, text="(Time before alarm triggers)").pack(side=tk.LEFT, padx=5)

        row += 1
        ttk.Label(settings_frame, text="Monitoring Phone:").grid(row=row, column=0, sticky=tk.W, pady=5, padx=5)
        ...
        power_status_label = ttk.Label(
            system_control_frame, textvariable=self.system_power_var, font=("Arial", 10, "bold")
        )
        power_status_label.pack(side=tk.LEFT, padx=10)

        ttk.Button(system_control_frame, text="Turn On", command=self.turn_system_on, width=12).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(system_control_frame, text="Turn Off", command=self.turn_system_off, width=12).pack(
            side=tk.LEFT, padx=5
        )
        ...
        ttk.Button(reset_frame, text="Reset System", command=self.reset_system, width=15).pack(side=tk.RIGHT, padx=5)

        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill=tk.X, pady=20)

        ttk.Button(button_frame, text="Save Settings", command=self.save_settings, width=20).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Cancel", command=self.cancel_changes, width=20).pack(side=tk.LEFT, padx=5)
    ...
    def turn_system_on(self):
        result = messagebox.askyesno("Turn System On", "Are you sure you want to turn the system on?")
        if result:
    ...
    def turn_system_off(self):
        result = messagebox.askyesno("Turn System Off", "Are you sure you want to turn the system off?")
        if result:
...
class PasswordChangeDialog(tk.Toplevel):
    ...
    def __init__(self, parent, password_type):
        ...
        ttk.Button(button_frame, text="OK", command=self.ok_clicked, width=12).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Cancel", command=self.cancel_clicked, width=12).pack(side=tk.LEFT, padx=5)
